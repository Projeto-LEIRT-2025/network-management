...
class RouterConfigurationServiceTest {
    ...
    fun `set an ip address should succeed`() {
        ...
        `when`(routerConfiguration.login()).thenReturn(true)
        `when`(routerConfiguration.setIpAddress(interfaceName, ipAddress, 24))
            .thenReturn(Response(raw = "", data = Unit))

        routerConfigurationService.setIpAddress(router.id, username, password, interfaceName, ipAddress, 24)

        verify(routerConfiguration).setIpAddress(interfaceName, ipAddress, 24)
    }
